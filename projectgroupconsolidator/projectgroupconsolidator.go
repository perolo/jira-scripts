...
func ProjectGroupConsolidator(propPtr string) { //nolint:gocyclo
	...
	for _, group := range Groups {
		...
		project, _, err3 := jiraClient.Project.Get(group.projectName)
		excelutils.Check(err3)
		...
		if closedDown {
			...
			if group.Group != "" {
				...
				for contjira {
					...
					jiramembers, _, err2 := jiraClient.Group.GetWithOptionsWithContext(context.Background(), group.Group, &jira.GroupSearchOptions{StartAt: startjira, MaxResults: maxjira})
					excelutils.Check(err2)
